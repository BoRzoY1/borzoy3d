...
// --- НОВАЯ/ОБНОВЛЕННАЯ ФУНКЦИЯ: Преобразование внешних ссылок и подготовка локальных ---
...
        // Используйте в админ-панели ссылки типа: https://www.dropbox.com/s/abcdef123456/video.mp4?dl=0
...
    }
    
    // 3. Возвращаем как есть (для локальных файлов на GitHub Pages)
    return link;
}
...
        videos: [
            // Оставьте эти пути, если используете локальные файлы GitHub Pages:
...
        videos: [
            // ИЛИ вставьте здесь прямую ссылку Dropbox, если проверяете:
            // { path: 'https://www.dropbox.com/s/abcdef123456/mage_render.mp4?dl=0', comment: 'Финальный рендер с Dropbox.' }
            { path: 'videos/mage_render.mp4', comment: 'Финальный рендер в Marmoset Toolbag.' }
        ]
    }
];
...
// --- Функции Рендеринга, Админки и Обсервер (Без изменений) ---
...
            // КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Используем новую функцию для обработки ссылок
            const videoSource = transformExternalLink(videoItem.path);

            const video = document.createElement('video');
            video.src = videoSource;
            // !!! ГЛАВНОЕ ИСПРАВЛЕНИЕ: ЯВНО УКАЗЫВАЕМ ТИП МЕДИА !!!
            // Это помогает, когда MIME-тип не передается от внешнего хоста (Dropbox, GD)
            // или некорректно определяется на GitHub Pages.
            if (videoSource.toLowerCase().endsWith('.mp4') || videoSource.includes('dl.dropboxusercontent.com')) {
